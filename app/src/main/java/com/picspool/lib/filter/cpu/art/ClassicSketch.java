...
public class ClassicSketch {
    public static Bitmap filter(Resources resources, Bitmap bitmap, String str, String str2) {
        return MaskFilter.filter(MaskFilter.filter(GrayscaleFilter.filter(bitmap, false), DMBitmapUtil.getImageFromAssetsFile(resources, str2), PorterDuff.Mode.SCREEN, true), DMBitmapUtil.getImageFromAssetsFile(resources, str), PorterDuff.Mode.MULTIPLY, true);
    }
