...
public class DM_FontAdapter extends BaseAdapter {
    ...
    private List<Typeface> tfList = DMInstaTextView.getTfList();
    ...
    public DM_FontAdapter(Context context) {
    ...
    private class BtnFontClickListener implements View.OnClickListener {
        ...
        public void onClick(View view) {
            ...
            if (DM_FontAdapter.this.fontManager.getRes(intValue) != null) {
                DMWBFontRes res = DM_FontAdapter.this.fontManager.getRes(intValue);
                if (res.getLocationType() == DMWBRes.LocationType.ASSERT) {
                    DM_FontAdapter.this.editText.setTextTypeface((Typeface) DM_FontAdapter.this.tfList.get(intValue));
                    DM_FontAdapter.this.editText.getTextDrawer().setTypefaceIndex(intValue);
                    DM_FontAdapter.this.setSelection(intValue);
                } else if (res.getLocationType() == DMWBRes.LocationType.ONLINE) {
                    try {
                        if (res.isContentExits()) {
                            DM_FontAdapter.this.editText.setTextTypeface(Typeface.createFromFile(res.getFontFileName()));
                            DM_FontAdapter.this.setSelection(intValue);
                        } else {
                            File file = new File(DM_FontAdapter.this.mContext.getCacheDir() + "/picsjoin/" + res.getFontUrl().split("/")[res.getFontUrl().split("/").length - 1]);
                            if (!file.exists()) {
                                return;
                            }
                            DM_FontAdapter.this.editText.setTextTypeface(Typeface.createFromFile(file.getAbsolutePath()));
                            DM_FontAdapter.this.setSelection(intValue);
                        }
    ...
    class DownloadFile extends AsyncTask<String, Integer, String> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            DM_FontAdapter.this.isLonding = true;
        }
        ...
        */
        public String doInBackground(String... r15) {
            ...
            */
            throw new UnsupportedOperationException("Method not decompiled: com.picspool.instatextview.edit.DM_FontAdapter.DownloadFile.doInBackground(java.lang.String[]):java.lang.String");
        }
        ...
        public void onPostExecute(String str) {
            ...
            if (!str.isEmpty()) {
                try {
                    ...
                    if (createFromFile != null && !DM_FontAdapter.this.tfList.contains(createFromFile)) {
                        DM_FontAdapter.this.tfList.add(createFromFile);
                        DM_FontAdapter.this.notifyDataSetChanged();
                    }
                } catch (Exception unused) {
                }
            }
            DM_FontAdapter.this.isLonding = false;
        }
    }
}
