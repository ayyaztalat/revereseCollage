...
public class DM_BasicColorView extends FrameLayout {
    ...
    public DM_BasicColorView(Context context) {
        super(context);
        iniView(context);
    }

    public DM_BasicColorView(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
        iniView(context);
    }

    public DM_BasicColorView(Context context, AttributeSet attributeSet, int i) {
    ...
    public void setColorListener(final DM_TextFixedView dM_TextFixedView) {
        ...
        this.colorGalleryView.setListener(new OnDMColorChangedListener() { // from class: com.picspool.instatextview.textview.DM_BasicColorView.1
            private boolean iniFlag = false;

            @Override // com.picspool.lib.widget.listener.OnDMColorChangedListener
            public void onColorChanged(int i) {
                DM_TextFixedView dM_TextFixedView2;
                int i2 = 0;
                while (true) {
                    if (!this.iniFlag || i2 >= DMSysColors.length) {
                        break;
                    } else if (i != DMSysColors.getColor(i2) || (dM_TextFixedView2 = dM_TextFixedView) == null || dM_TextFixedView2.getTextDrawer() == null) {
                        i2++;
                    } else {
                        dM_TextFixedView.setTextColor(i);
                        dM_TextFixedView.getTextDrawer().setPaintColorIndex(i2);
                        if (DM_BasicColorView.this.bcAdapter != null) {
                            DM_BasicColorView.this.bcAdapter.iniSelectionItem();
                        }
                    }
                }
                if (this.iniFlag) {
