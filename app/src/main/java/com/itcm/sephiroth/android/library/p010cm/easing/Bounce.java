...
/* renamed from: itcm.sephiroth.android.library.cm.easing.Bounce */
...
public class Bounce implements Easing {
    ...
    public double easeIn(double d, double d2, double d3, double d4) {
        return (d3 - easeOut(d4 - d, FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE, d3, d4)) + d2;
    }
    ...
    public double easeInOut(double d, double d2, double d3, double d4) {
        double easeOut;
        if (d < d4 / 2.0d) {
            easeOut = easeIn(d * 2.0d, FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE, d3, d4) * 0.5d;
        } else {
            easeOut = (easeOut((2.0d * d) - d4, FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE, d3, d4) * 0.5d) + (d3 * 0.5d);
        }
        return easeOut + d2;
    }
}
