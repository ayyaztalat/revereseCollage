...
public class NotificationUtils {
    ...
    public static boolean areNotificationsEnabled() {
        return NotificationManagerCompat.from(C0879Utils.getApp()).areNotificationsEnabled();
    }

    public static void notify(int i, C0879Utils.Consumer<NotificationCompat.Builder> consumer) {
        notify(null, i, ChannelConfig.DEFAULT_CHANNEL_CONFIG, consumer);
    }

    public static void notify(String str, int i, C0879Utils.Consumer<NotificationCompat.Builder> consumer) {
        notify(str, i, ChannelConfig.DEFAULT_CHANNEL_CONFIG, consumer);
    }

    public static void notify(int i, ChannelConfig channelConfig, C0879Utils.Consumer<NotificationCompat.Builder> consumer) {
        notify(null, i, channelConfig, consumer);
    }

    public static void notify(String str, int i, ChannelConfig channelConfig, C0879Utils.Consumer<NotificationCompat.Builder> consumer) {
        NotificationManagerCompat.from(C0879Utils.getApp()).notify(str, i, getNotification(channelConfig, consumer));
    }

    public static Notification getNotification(ChannelConfig channelConfig, C0879Utils.Consumer<NotificationCompat.Builder> consumer) {
        if (Build.VERSION.SDK_INT >= 26) {
            ((NotificationManager) C0879Utils.getApp().getSystemService("notification")).createNotificationChannel(channelConfig.getNotificationChannel());
        }
        NotificationCompat.Builder builder = new NotificationCompat.Builder(C0879Utils.getApp());
        if (Build.VERSION.SDK_INT >= 26) {
    ...
    public static void cancel(String str, int i) {
        NotificationManagerCompat.from(C0879Utils.getApp()).cancel(str, i);
    }

    public static void cancel(int i) {
        NotificationManagerCompat.from(C0879Utils.getApp()).cancel(i);
    }

    public static void cancelAll() {
        NotificationManagerCompat.from(C0879Utils.getApp()).cancelAll();
    }
    ...
    private static void invokePanels(String str) {
        try {
            Class.forName("android.app.StatusBarManager").getMethod(str, new Class[0]).invoke(C0879Utils.getApp().getSystemService("statusbar"), new Object[0]);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static class ChannelConfig {
        public static final ChannelConfig DEFAULT_CHANNEL_CONFIG = new ChannelConfig(C0879Utils.getApp().getPackageName(), C0879Utils.getApp().getPackageName(), 3);
